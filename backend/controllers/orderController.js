...
export const createOrder = async (req, res) => {
    try {
        ...
        if (paymentMethod === 'online') {
            // Calculate tax for each item (5% of item price)
            const session = await stripe.checkout.sessions.create({
                payment_method_types: ['card'],
                mode: 'payment',
                line_items: orderItems.map(o => {
                    const itemPrice = o.item.price;
                    const itemTax = itemPrice * 0.05; // 5% tax
                    const totalItemPrice = itemPrice + itemTax;
                    ...
                    return {
                        price_data: {
                            ...
                            product_data: { 
                                name: o.item.name,
                                description: `Price: ₹${itemPrice.toFixed(2)} + Tax (5%): ₹${itemTax.toFixed(2)}`
                            },
                            unit_amount: Math.round(totalItemPrice * 100) // Convert to paisa
                        },
                        quantity: o.quantity
                    };
                }),
