...
const Orders = () => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get(
          'https://friendlyfoodie-backend.onrender.com/api/orders/getall',
          {
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      await axios.put(`https://friendlyfoodie-backend.onrender.com/api/orders/getall/${orderId}`, { status: newStatus });
              ...
              <table className={tableClasses.table}>
                ...
                <tbody>
                  {orders.map(order => {
                    ...
                    const subtotal = order.subtotal ?? order.items.reduce((s, i) => s + i.item.price * i.quantity, 0);
                    const tax = order.tax ?? (subtotal * 0.05); // 5% tax if not provided
                    const shipping = order.shipping ?? 0;
                    const totalPrice = order.total ?? (subtotal + tax + shipping);
                      ...
                      <tr key={order._id} className={tableClasses.row}>
                          ...
                          <div className="space-y-1 max-h-52 overflow-auto">
                            {order.items.map((itm, idx) => (
                              <div key={idx} className="flex items-center gap-3 p-2 rounded-lg">
                                <img src={`https://friendlyfoodie-backend.onrender.com${itm.item.imageUrl}`} alt={itm.item.name} className="w-10 h-10 object-cover rounded-lg" />
                        ...
                        <td className={tableClasses.cellBase}>
                          <div className="flex flex-col gap-1">
                            <div className="text-amber-300 text-lg">₹{totalPrice.toFixed(2)}</div>
                            <div className="text-xs text-amber-400/60">
                              <div>Subtotal: ₹{subtotal.toFixed(2)}</div>
                              <div>Tax: ₹{tax.toFixed(2)}</div>
                              {shipping > 0 && <div>Shipping: ₹{shipping.toFixed(2)}</div>}
                            </div>
                          </div>
                        </td>
                        <td className={tableClasses.cellBase}>
